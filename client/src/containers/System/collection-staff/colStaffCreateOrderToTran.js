...
class colStaffCreateOrderToTran extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      isOpen: false,
  ...
  componentDidMount() {
    ...
    this.props.getAllTransactions(this.props.userInfo.token);
    this.props.getAllCollections();
    this.buildFromColInfo();
    let transactionOptions = CommonUtils.buildSelectionOptions(this.props.arrTransactions);
    this.setState({
      optionSelectionCollections: transactionOptions,
    });
  }
  ...
  handleChangeSelectCollection = (selectedTran) => {
    this.setState({
      selectedTran: selectedTran,
    });
    try {
      let colZipCode = selectedTran.value;
      const { arrTransactions } = this.props;
      if (arrTransactions && arrTransactions.length > 0) {
        for (let i = 0; i < arrTransactions.length; i++) {
          if (colZipCode === arrTransactions[i].zip_code) {
            this.setState({
              r_colInfo: arrTransactions[i],
            });
            break;
          }
        }
  ...
  render() {
    let { arrParcelsToSendCol } = this.props;
    let { s_colInfo, r_colInfo, optionSelectionCollections, selectedTran } = this.state;
                ...
                <Row className="mb-5">
                  ...
                  <Col>
                    ...
                    <Select
                      ...
                      placeholder={<div>Your Transaction</div>}
                    />
                    <Form.Control
...
export default connect(mapStateToProps, mapDispatchToProps)(colStaffCreateOrderToTran);
