...
import ProgressOrder from './HomePage/progress_order/ProgressOrder.js';
          ...
          <div className="main-container">
            <div className="content-container">
              ...
              <Switch>
                ...
                <Route path={'/magic-post/progress/parcel/:parcel_id'} component={ProgressOrder} />
                {userInfo && userInfo.role === 'ADMIN' && (
                  <>
                    <Redirect to={path.ADMIN_CREATE_ACCOUNT} />
                    <Route path={path.SYSTEM} component={userIsAuthenticated(BoardOfDirector)} />
                  </>
                )}
                {userInfo && userInfo?.role === 'TRANSACTION_ADMIN' && (
                  <>
                    <Redirect to={path.TRANSACTION_ADMIN_CREATE_ACCOUNT} />
                    <Route path={path.TRANSACTION_ADMIN} component={userIsAuthenticated(RouteTransactionManager)} />
                  </>
                )}
                {userInfo && userInfo?.role === 'COLLECTION_ADMIN' && (
                  <>
                    <Redirect to={path.COLLECTION_ADMIN_CREATE_ACCOUNT} />
                    <Route path={path.COLLECTION_ADMIN} component={userIsAuthenticated(RouteCollectionManager)} />
                  </>
                )}
                {userInfo && userInfo?.role === 'TRANSACTION_STAFF' && (
                  <>
                    <Redirect to={path.TRANSACTION_STAFF_CREATE_PARCEL} />
                    <Route path={'/transaction-staff'} component={userIsAuthenticated(RouteTransactionStaff)} />
                  </>
                )}
                {userInfo && userInfo?.role === 'COLLECTION_STAFF' && (
                  <>
                    <Redirect to={path.COLLECTION_STAFF_MANAGE_PARCEL} />
                    <Route path={'/collection-staff'} component={userIsAuthenticated(RouteCollectionStaff)} />
                  </>
                )}
              </Switch>
              {/* </Scrollbars> */}
